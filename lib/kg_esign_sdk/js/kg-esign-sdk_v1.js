function _runApp() {
  ...
  var modal = document.getElementById("modalUploadPhoto");
  var wrapper = document.getElementById("signature-pad");
    var clearButton = modal.querySelector("[data-action=clear]"); 
    // var changeColorButton = wrapper.querySelector("[data-action=change-color]");
    // var undoButton = wrapper.querySelector("[data-action=undo]");
    var savePNGButton = modal.querySelector("[data-action=save-png]");
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (aResult.return == "ok") {
                    $(".divPhoto img").attr("src", "photos/" + aResult.photo_file);
                    $(".divSign img").attr("src", "signatures/" + aResult.sign_file);

                    $("input[data-key=Photo]").val(aResult.photo_file);
                    $("input[data-key=Signature]").val(aResult.sign_file);
                    $(".divSign img").show();
                } else {
                    _systemAlert('system');
                }
