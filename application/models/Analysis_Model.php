<?php
    ...
    class Analysis_Model extends CI_Model {
        ...
        public function proc_analysis($vAction, $aData) {

            switch ($vAction) {
                case 'get-ques':

                    $nPartId        =   $this->session->userdata('sess_partid');
                    $aQuestions     =   $this->hatvclib->GetAnalysisQues($aData);
                ...
                case 'get-ans':
                    ...
                    $sSelect =  "<select class='form-control input-sm selAns' data='req'>";
                    $sSelect.=  "<option value=''>";

                    if ($aData['quesid'] != "region" && $aData['quesid'] != "age" && $aData['quesid'] != "gender" && $aData['quesid'] != "occupation" && $aData['quesid'] != "educattain") {
                        if (sizeof($aReturn) > 0) 
                        {
                            if ($aReturn[0]->ans_type == "range") 
                            {
                                $aRange =   explode("-", $aReturn[0]->ans_data);

                                $x      = intval($aRange[0]);
                                while ($x <= $aRange[1]) {

                                    $sSelect.=  "<option value='".$x."'>".$x;
                                    $x += intval($aReturn[0]->ans_increment);
                                }
                            }
                            else
                            {
                                ...
                                foreach ($aReturn as $key => $oValue) {
                                    $sSelect.=  "<option value='".$oValue->ans_data."'>".$oValue->ans_data;
                                }
                            }

                            $sSelect.=  "</select>";

                            $sObjRet['message'] = $sSelect;
                        } else {
                            $sObjRet['message'] = "";
                        }
                    }
                        ...
                        switch ($aData['quesid']) {
                            case "gender"       : $sObjRet['message'] = $this->hatvclib->DropGender('selGender', ''); break;
                            case "age"          : $sObjRet['message'] = "<input name='txtAge' id='txtAge' type='text' class='form-control input-sm' onkeypress='return rangeOnly(event)'>"; break;
                            case "occupation"   : $sObjRet['message'] = $this->hatvclib->DropOccupation('selOccupation', ''); break;
                            case "educattain"   : $sObjRet['message'] = $this->hatvclib->DropEducAttain('selEducAttain', ''); break;
                            case "region"       : $sObjRet['message'] = $this->hatvclib->DropDowns('region', array('region', ''), ''); break;
                        }
                    }
